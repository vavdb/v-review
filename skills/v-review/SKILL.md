...
27. **Names that abbreviate for nothing, or lie about what they hold.** A name is the only documentation that every reader is forced to read. Two failures, both cheap to fix and both compounding:

   **(a) Abbreviation that saves keystrokes and costs comprehension.** `Cols`, `Ctx`, `Cfg`, `Req`, `Res`, `Tmp`, `Val`, `Idx`, `Mgr`, `Svc`, `Attrs`, `Params`, `Prev`, `Cnt`. The full word is one autocomplete away and reads correctly out loud in a review. Flag the abbreviation unless it is **established domain vocabulary** (`Id`, `Url`, `Http`, `Sql`, `Db`, `Utc`, `Api`, `Json`), a **conventional loop index** (`i`, `j`), or a **domain term of art** the codebase already uses consistently (`e1RM`, `Rpe`, `Ttl`). "It was shorter" is not a defence; "the reader would have to expand it mentally to know what it is" is the test.

   **(b) Names that misdescribe the type or the role — worse, because they actively mislead.**
   - **Type-lying suffixes.** `ColList` holding a comma-joined `string`; `userMap` holding a `List<>`; `Items` holding one item; `Count` holding a `bool`; `xs` holding a dictionary. The reader plans `.Select(...)` against a name that promised a collection, and finds out at the compiler — or worse, at runtime through a `string` that happens to be enumerable as chars.
   - **Wrong-domain words.** `Mutable` naming a set of columns an upsert overwrites, inside a codebase built on immutable records — the word means something specific there and this isn't it.
   - **Stutter.** `SetSql.ColumnsSql`, `UserService.UserServiceOptions`, `OrderRepository.OrderRepositoryCache`. The scope already said it; the member should not repeat it.
   - **Test-only members on production types.** `ColumnsForTest`, `InternalsForTesting`, `PublicForUnitTests`. The visibility is the finding, not the name — use `InternalsVisibleTo` and let the test read the real member.
   - **Names that only parse if you already know the idiom.** `ConflictAssignments` for the `SET` block of an `ON CONFLICT DO UPDATE` reads as noise to anyone who hasn't written a SQLite upsert; `OverwriteWithIncoming` says what happens. Domain terms are fine when the domain is the codebase's; they are not fine when they are one library's internal jargon.
